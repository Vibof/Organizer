...
```
Тут task_5 - это папка в локальном репозитории, содержащая файл task_5.jl подготовленный для отправки на сайт (или просто подготовленный пока только для сохранения в локальном репозитории). Если папка task_5 содержит другие важные файлы, то с ними надо поступить аналогично.
...
> git add task_5.jl
```
1. Фиксация текущего состояния проиндексированных файлов репозитория с помощью команды
```
